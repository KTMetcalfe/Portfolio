...
function Planet({ distance, size, speed, rotation, name }) {
  ...
  // Allows for physics
  const [planetRef, planetApi] = useSphere(() => ({
  ...
  useFrame(() => {
    ...
    if (
      ...
      state.selectedRef.current !== null &&
      state.selectedRef.current.geometry.parameters !== null
    ) {
      const goodPos = new THREE.Vector3(
        state.selectedRef.current.position.x,
        state.selectedRef.current.position.y +
          2 * state.selectedRef.current.geometry.parameters.radius,
        state.selectedRef.current.position.z
      ).multiplyScalar(
        1 +
          5 *
            (state.selectedRef.current.geometry.parameters.radius /
              state.selectedRef.current.position.distanceTo(
    ...
    planetApi.rotation.set(
      ...
      (planetRef.current.rotation.y += THREE.MathUtils.degToRad(
        360 / rotation
      )),
  ...
  return (
    <mesh
      ...
      onClick={() => {
        ...
        if (name !== 'Sun' && name !== state.selectedRef?.current.name) {
          setState({ selectedRef: planetRef, shouldLerp: true });
        }
      }}
    >
      <Text
  ...
  useFrame(() => {
    ...
    if (
      ...
      if (state.shouldLerp === true) {
        camera.position.lerp(goodPos, 0.1);
        setTimeout(() => {
          setState({
...
function App() {
  ...
  // Main renderer
  return (
    <div id='main' className='App -z-10'>
      <div className='overlay z-10 w-1/2'>
        ...
        <button
          disabled={state.shouldLerp === true}
        ...
        <input
          ...
          onInput={(e) => setSecPerYear(Number(e.target.value))}
        ></input>
        ...
        <label className='overlayText'>
          {secPerYear === 31536000
            ...
            : secPerYear === 365
            ? '(1 second = 1 day)'
            : (365 / secPerYear).toFixed(2)}
          x
        </label>
        <button
          disabled={secPerYear === 31536000}
          className='overlayText'
          onClick={() => setSecPerYear(31536000)}
        >
          Realtime
        </button>
      </div>
